...
class AutoUsernameSettingsForm extends ConfigFormBase {
    ...
    foreach ($punctuation as $name => $details) {
      if ($details['value'] == $separator) {
        ...
        if ($action == AUN_PUNCTUATION_REMOVE) {
          \Drupal::messenger()->addError($this->t('You have configured the @name to be the separator and to be removed when encountered in strings. You should probably set the action for @name to be "replace by separator".', ['@name' => $details['name']]));
        }
      }
    }
  }
  ...
  public static function autoUsernamePatternprocessor($account) {
    $output = '';
    $pattern = \Drupal::config('auto_username.settings')->get('aun_pattern');
    if (trim($pattern)) {
      ...
      if (\Drupal::config('auto_username.settings')->get('aun_php')) {
  ...
  public static function autoUsernameGenerateUsername(&$account) {
    ...
    if (($account->id() == 1 || $account->hasRole('propietario')) && !$account->isNew()) {
  ...
  public static function autoUsernameCleanstring($string) {
    ...
    if (!isset($cache)) {
      $cache = [
        'separator' => \Drupal::config('auto_username.settings')->get('aun_separator'),
        'transliterate' => \Drupal::config('auto_username.settings')->get('aun_transliterate') && \Drupal::moduleHandler()->moduleExists('transliteration'),
        'punctuation' => [],
        'reduce_ascii' => (bool) \Drupal::config('auto_username.settings')->get('aun_reduce_ascii'),
        'ignore_words_regex' => FALSE,
        'replace_whitespace' => (bool) \Drupal::config('auto_username.settings')->get('aun_replace_whitespace'),
        'lowercase' => (bool) \Drupal::config('auto_username.settings')->get('aun_case'),
        'maxlength' => min(\Drupal::config('auto_username.settings')->get('aun_max_component_length'), self::autoUsernameGetSchemaNameMaxlength()),
      ];

      // Generate and cache the punctuation replacements for strtr().
      $punctuation = self::autoUsernamePunctuationChars();
      foreach ($punctuation as $name => $details) {
        $action = \Drupal::config('auto_username.settings')->get('aun_punctuation_' . $name);
        switch ($action) {
      ...
      $ignore_words = \Drupal::config('auto_username.settings')->get('aun_ignore_words');
  ...
  public static function autoUsernameCleanSeparators($string, $separator = NULL) {
    ...
    if (!isset($separator)) {
      if (!isset($default_separator)) {
        $default_separator = \Drupal::config('auto_username.settings')->get('aun_separator');
      }
